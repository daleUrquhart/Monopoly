     ...
     void buy(Property p, int bid, Game game) {
         ...
         if(p.isMortgaged()) {
             ...
             if(canAfford((int) (bid+p.getMortgageValue()*1.1))) {
                 ...
                 if (game.boolInput("Buy", p.getName()+" is mortgaged, would you like to unmortgage it now, or pay it later?", "UnMortgage price: "+(int) (p.getMortgageValue() * 1.1)+"Intreset only price: "+(int) (p.getMortgageValue() * 0.1)+".")) {
                     p.unMortgage();
                 } else {
